...
SYSTEM_TOOLS_TO_VALIDATE = {"Bash", "Write", "Edit"}

# Table de bijection code ↔ ontologie (principe ontology-first).
# Plan 06-04 follow-up (2026-04-25): la bijection vit désormais sur chaque
# instance de l'ontologie, dans la propriété `implementation.files`. Le
# fichier `counsel_proxy/code_ontology_map.json` n'est plus autoritatif —
# l'ontologie est la source de vérité unique (DDD ubiquitous language).


def _build_file_to_entity_map(graph) -> dict[str, str]:
    """Walk every instance and return {basename: entity_id} from `implementation.files`.

    Reads directly from the in-memory ontology graph; no separate file lookup.
    """
    out: dict[str, str] = {}
    for node_id, node in getattr(graph, "nodes", {}).items():
        # OntologyGraph.nodes values may be dicts or objects depending on impl;
        ...
        if not isinstance(props, dict):
            continue
        impl = props.get("implementation")
        if not isinstance(impl, dict):
            continue
        for repo_path in impl.get("files", []) or []:
            if isinstance(repo_path, str) and repo_path:
                out[Path(repo_path).name] = node_id
    return out


def check_code_ontology_bijection(tool_name: str, tool_input: dict, graph) -> list[str]:
    """Warn (not block) when a written/edited .py file has no ontology mapping."""
    ...
    mapped = _build_file_to_entity_map(graph)
    if fname not in mapped:
        return [
            f"[BIJECTION] '{fname}' has no `implementation.files` entry on any "
            f"ontology instance. Add the file under the relevant entity's "
            f"`properties.implementation.files` array (ontology-first principle) "
            f"or confirm this file has no semantic equivalent in the graph."
        ]
    entity_id = mapped[fname]
    if entity_id not in graph.nodes:
        return [
            f"[BIJECTION] '{fname}' maps to '{entity_id}' "
            f"but this entity does not exist in the ontology. Add it first."
        ]
    return []
...
def main():
    ...
    # Bijection check — warn if a code file has no ontology mapping
    bijection_warnings = check_code_ontology_bijection(tool_name, tool_input, graph)
    for w in bijection_warnings:
